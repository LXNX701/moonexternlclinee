...
struct ThreeOneOSFiveApp: App {
    ...
    init() {
        ...
        setupLogCapture()
        log("app: Moon Place launching — iOS \(AppInfo.osVersion) (\(AppInfo.osBuild)) \(AppInfo.machineName)")
    }
    ...
    var body: some Scene {
        WindowGroup {
            ZStack {
                if showWelcome {
                    MoonWelcomeView(isPresented: $showWelcome)
                        .transition(.opacity)
                } else if !isAuthenticated {
                    MoonLoginView(isAuthenticated: $isAuthenticated)
                        .transition(.opacity)
                } else {
                    mainContentView
                }
